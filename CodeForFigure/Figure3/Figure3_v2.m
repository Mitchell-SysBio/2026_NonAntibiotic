...
[sortFrequencey,indx] = sort(frequencyAll,'descend'); 
f1 = figure;hold on;
plot(sort(frequencyAll,'descend'), 'k', 'Linewidth',2)
area(sort(frequencyAll,'descend'))
text(1:10+0.4,sortFrequencey(1:10),uniGene(indx(1:10)), 'HorizontalAlignment','left');
scatter(1:10+0.4,sortFrequencey(1:10));
...
for i = orgInd' 
    ...
    scatter(x(positionMutation(I1(S~=1))), y(positionMutation(I1(S~=1))),50, cols(S(S~=1),:),'filled');
    for i1 = I1(S>=3)'
        text(x(positionMutation(i1))+100, y(positionMutation(i1))+100, mutT{i1,"gene"}{1})
    end 
    ...
    storeNmut{nMarker,1} = allDrugGeneData{i,2};
    storeNmut{nMarker,2} = sum(S);
    nMarker = nMarker +1;
end
...
%% Supplementary Figure: autoresistance vs # of mutations 
cd ../ 
load("AutoResistanceData_2026-04-24.mat", "autoResTable")
cd Figure3_SuppFig5/
figure; hold on;
for s1 = 1:height(storeNmut)
    drug1 = storeNmut{s1,1};
    % accomodating different naming between variables 
    if strcmp(drug1, 'TetracyclineHCl')
        drug1 = 'Tetracycline';
    elseif strcmp(drug1, 'ColistinSulfate')
        drug1 = 'Colistin';
    elseif strcmp(drug1, 'BleomycinSulfate')
        drug1 = 'Bleomycin';
    elseif strcmp(drug1, 'Ciclopiroxethanolamine')
        drug1 = 'Ciclopirox';
    elseif strcmp(drug1, 'Deferoxaminemesylate')
        drug1 = 'Deferoxamine';
    elseif strcmp(drug1, 'MitomycinC')
        drug1 = 'Mitomycin';
    elseif strcmp(drug1, 'PentamidineIsethionate')
        drug1 = 'Pentamidine';
    end
    idx = find(contains(autoResTable.drug, drug1));
    LFCavgIC50 = autoResTable{idx, "avgLFC_IC"};
    scatter(LFCavgIC50, storeNmut{s1,2},'filled')
    if sum(contains(superBugs, drug1))>0
        text(LFCavgIC50+0.05, storeNmut{s1,2},drug1)
    end 
end 
grid on; 
xlabel('average IC50 log2 fold change')
